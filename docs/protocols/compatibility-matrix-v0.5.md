...
| Browser runtime | `bpa.browser/2@2.0.0` | Session, page observation and exact-tab Command state | v1 receives an explicit upgrade error and cannot execute v2 assets |
...
| Design capture | Exact Design Mode Grant plus built-in read-only capture Node | Evidence-backed PageSnapshot | `bpa.browser/2@2.0.0` binds the exact observed tab; snapshot bodies use Evidence references |
...
- `bpa.browser/2@2.0.0`, Evidence v1 and `bpa.workflow-ir/2` are the current
  identifiers. Browser v1 is not an executable compatibility path.
