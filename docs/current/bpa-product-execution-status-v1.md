...
- 本代码候选已将 `doudian.alliance-retired-products-monitor` 升为 `3.0.5`，逐店浏览器
...
- 已有 Doudian Alliance Adapter `2.0.4`、Extension 执行端与 Core/Provider fixture 测试；
- Windows WorkBuddy Skill、安装器、业务打包/验包器和两个专属 CI job 继续保留，作为
  Mac 真实 canary 前的现网修复与回退能力；过渡安装器同步发布浏览器 Node `2.0.4`、Workflow
  `3.0.5` 当前闭包；该版本把店铺发现与逐店扫描的单次调用预算明确冻结为 5 分钟，避免旧发布
...
走通清退商品 `2.0.4`、体验分 `2.0.0`、库存 `2.0.0` 的 Trigger、资源绑定、IR2、Provider、
