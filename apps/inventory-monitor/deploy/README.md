...
7. Verify `127.0.0.1:17650`, the `0600` Unix socket and launch-URL file, Trigger/Run state, lease rows, and absence of DingTalk network requests. Requests from loopback or the fixed company LAN `192.168.3.0/24` may establish a signed employee session without a launch token; other source networks remain rejected. Keep the launch URL as a recovery path and never print its token.
